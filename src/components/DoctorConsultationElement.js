...
export default function DoctorConsultationElement({item, navigation}) {
  ...
  useEffect(() => {
    async function getAllInfos() {
      try {
        axios
          .get(`${IP}/api/doctor/getAllInfos/${item.doctorId}`)
          .then(infos => {
            console.log(infos.data.user);
            setDoctor(infos.data.user);
          });
      } catch (error) {
        console.log(error);
      }
      ...
      <View
        ...
        <View style={{flexDirection: 'row'}}>
          ...
          <View style={{flexDirection: 'column'}}>
            <View style={{flexDirection: 'row'}}>
              <Text style={styles.buttonNoun}>Dr.{doctor.firstName}</Text>
              <Text style={styles.buttonNoun}> {doctor.lastName}</Text>
            </View>
            <View style={{flexDirection: 'row'}}>
